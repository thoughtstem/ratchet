...
(define (run-code-button parent input-editor output-editor)
  (define (color-bg-with-text text)
    [define bm (make-object bitmap% 100 40)]
    [define dc (make-object bitmap-dc% bm)]
    (send dc set-brush (make-object brush% (make-color 120 220 82) 'solid))
    (send dc draw-rectangle 0 0 100 40)
    (send dc draw-text text 35 10)
    bm)
  
  (define b
    (new button%
       [parent parent]
       [label (color-bg-with-text "RUN")]
       [min-width 100]	 
       [min-height 40]
       [callback
        (thunk*
